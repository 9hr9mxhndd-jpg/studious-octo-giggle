...
export function LandingPage() {
  ...
  const { auth, user, playlists, setPlaylists, selectPlaylist, selectedPlaylistId, importSongs, setActiveSource, activeSource, songs, matches } = useAppStore();

  const [loginError, setLoginError] = useState<string>();
  const [loading, setLoading] = useState(false);
  const [starting, setStarting] = useState(false);
  const [selectedId, setSelectedId] = useState<string | undefined>(selectedPlaylistId);
  ...
  const hasResume = Boolean(activeSource && songs.length > 0 && matches.length > 0);

  async function handleStart() {
    if (!selectedId || !selectedPlaylist) return;
    setStarting(true);
    try {
      const imported = await importPlaylistTracks(selectedId, auth?.accessToken);
    ...
    return (
      <div className="space-y-8 pt-4">
        <div className="text-center">
          <p className="mb-3 text-xs uppercase tracking-widest text-warm-400">Elo Rating · Top-K Matching</p>
          ...
          <p className="mx-auto mt-4 max-w-xs text-sm leading-relaxed text-warm-500">
            좋아하는 곡 수백 개를 직접 비교해서 순위를 매기는 건 불가능에 가깝습니다. Elo와 Top-K 샘플링으로 가능하게 만들었어요.
          </p>
          <div className="mt-6">
            <button
              type="button"
              onClick={() => {
                ...
                void signInWithSpotify().catch((e: unknown) => {
                  setLoginError(e instanceof Error ? e.message : '로그인 중 오류가 발생했어요');
                });
              }}
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            { icon: '🗂️', title: '3단계 분류', desc: '선호·보통·기타로 사전 분류해 수렴 속도 향상' },
            { icon: '⚡', title: 'Top-K 매칭', desc: '가중 샘플링으로 반복 없는 최적 비교 쌍 선택' },
            { icon: '🏆', title: 'Elo 랭킹', desc: '비교 결과가 실시간으로 순위에 반영됩니다' },
          ].map((f) => (
            <div key={f.title} className="rounded-xl border border-warm-200 bg-white p-3.5">
              <div className="mb-1.5 text-base">{f.icon}</div>
              <div className="text-xs font-medium text-warm-800">{f.title}</div>
              <div className="mt-1 text-[10px] leading-relaxed text-warm-400">{f.desc}</div>
            </div>
          ))}
        </div>

        <p className="text-center text-xs text-warm-400">
          Premium: 전곡 재생 · Free: 30초 미리듣기<br />
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3 rounded-xl border border-warm-200 bg-white px-4 py-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-warm-400">Spotify {user.isPremium ? 'Premium' : 'Free'} 연결됨</p>
        </div>
        <span className={`rounded-full border px-2 py-0.5 text-[10px] font-medium ${user.isPremium ? 'border-amber-300 bg-amber-50 text-amber-700' : 'border-warm-200 bg-warm-100 text-warm-400'}`}>
      ...
      {hasResume && activeSource && (
        <div className="rounded-xl border border-brand-200 bg-brand-50 p-4">
          <div className="mb-3 flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-warm-800 truncate">{activeSource.name}</p>
              <p className="text-xs text-warm-500">{matches.length}회 비교 완료 · {songs.filter((s) => s.tier !== undefined).length}/{activeSource.trackCount}곡 분류</p>
            </div>
          </div>
          <div className="mb-3 h-1.5 overflow-hidden rounded-full bg-brand-100">
            <div
              className="h-full rounded-full bg-brand-500 transition-all"
              style={{ width: `${Math.min(100, Math.round((songs.filter((s) => s.tier !== undefined).length / activeSource.trackCount) * 100))}%` }}
            />
          </div>
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {playlists.map((playlist) => (
              ...
              >
                {playlist.imageUrl ? (
                  <img src={playlist.imageUrl} alt={playlist.name} className="h-10 w-10 shrink-0 rounded-lg object-cover" />
                ) : (
                ...
                <div className="flex-1 min-w-0">
                  <p className="truncate text-sm font-medium text-warm-800">{playlist.name}</p>
                  <p className="text-xs text-warm-400">{playlist.trackCount.toLocaleString()}곡</p>
                </div>
                <div className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border text-[10px] transition ${
                  selectedId === playlist.id ? 'border-warm-800 bg-warm-800 text-white' : 'border-warm-300 text-transparent'
                }`}>
                  ✓
                </div>
              </button>
        ...
        >
          <div>
            <p className="text-sm font-medium text-white">
              {selectedPlaylist ? `${selectedPlaylist.name}으로 소팅 시작` : '소스를 선택해주세요'}
            </p>
            {selectedPlaylist && (
              <p className="text-xs text-white/60">
                {selectedPlaylist.trackCount.toLocaleString()}곡 · {estimateSession(selectedPlaylist.trackCount)}
              </p>
            )}
          </div>
          <span className="text-white/70 text-lg">{starting ? '···' : '→'}</span>
        </button>
      </div>
    </div>
  );
